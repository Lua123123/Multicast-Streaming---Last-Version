...
def audio_stream():
    ...
    print("Client connected to audio stream server port", socket_address)
    data = b""
...
def audio_video():
    ...
    print("Client connected to audio video server port", (host_ip, port))
    data = b""
    payload_size = struct.calcsize("Q")
    while True:
        while len(data) < payload_size:
...
def video():
    client_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    host_ip = '127.0.0.1'  # Here Require CACHE Server IP
    # host_ip = '192.168.0.112'  # Here Require CACHE Server IP
    port = 9999
    client_socket.connect((host_ip, port))  # a tuple
